...
class ZscoreSustain(AbstractSustain):
    ...
    def _optimise_parameters(self, sustainData, S_init, f_init, rng):
        ...
        p_perm_k_weighted                   = p_perm_k * f_val_mat + 1e-250 #infinite fix
    ...
    def _plot_sustain_model_fix(self, *args, **kwargs):
        return ZscoreSustain.plot_positional_var_fix(*args, Z_vals=self.Z_vals, **kwargs)
    ...
    @staticmethod
    def plot_positional_var_fix(samples_sequence, samples_f, n_samples, Z_vals, biomarker_labels=None, ml_f_EM=None, cval=False, subtype_order=None, biomarker_order=None, title_font_size=12, stage_font_size=10, stage_label='SuStaIn Stage', stage_rot=0, stage_interval=1, label_font_size=10, label_rot=0, cmap="original", biomarker_colours=None, figsize=None, subtype_titles=None, separate_subtypes=False, save_path=None, save_kwargs={}):
        # Get the number of subtypes
        N_S = samples_sequence.shape[0] #
        # Get the number of features/biomarkers
        N_bio = Z_vals.shape[0] #10
        # Check that the number of labels given match
        if biomarker_labels is not None:
            assert len(biomarker_labels) == N_bio
        # Set subtype order if not given
        ...
        IX_select = np.nonzero(stage_zscore)[0]
        stage_zscore = stage_zscore[IX_select][None, :]
        zvalues =  stage_zscore[0] #np.unique(stage_zscore)
        N_z = len(zvalues)

        # Extract which biomarkers have which zscores/stages
        stage_biomarker_index = np.tile(np.arange(N_bio), (N_z,))
        stage_biomarker_index = stage_biomarker_index[IX_select]
        #print(stage_biomarker_index)

        # Colour definition for two levels
        colour_mat = np.array([[1, 0, 0], [1, 0, 1], [0, 0, 1], [0.5, 0, 1], [0, 1, 1], [0, 1, 0.5]])[:N_z]

        #if len(colour_mat) < len(zvalues):
        #   raise ValueError('Not enough colors for the number of levels.')
        ...
        for i in range(subtype_loops):
            ...
            for j in range(total_axes):
                ...
                this_samples_sequence = samples_sequence[subtype_order[i], :, :].T
                #print(this_samples_sequence)
                N = this_samples_sequence.shape[1]

                confus_matrix = (this_samples_sequence == np.arange(N)[:, None, None]).sum(1) / this_samples_sequence.shape[0]
                confus_matrix_c = np.ones((N_bio, N, 3))

                # Applying colors to each row without overlapping
                for k, level in enumerate(zvalues):
                    k = k//len(biomarker_labels)#int(k < 10)
                    alter_level = colour_mat[k] == 0 
                    #print(alter_level)
                    confus_matrix_zscore = confus_matrix[(stage_zscore == level)[0]]
                    confus_matrix_c[
                        np.ix_(
                            stage_biomarker_index[(stage_zscore == level)[0]], range(N),
                            alter_level
                        )
                    ] -= np.tile(
                        confus_matrix_zscore.reshape((stage_zscore == level).sum(), N, 1),
                        (1, 1, alter_level.sum())
                    )

                # Sorting rows based on the first appearance of the lowest level (red)
             #   first_level_indices = np.argmax(confus_matrix_c[:, :, 0] == 0, axis=1)
               # biomarker_order = np.argsort(first_level_indices)
                ...
                ax.set_xticklabels(stage_ticks+1, fontsize=stage_font_size, rotation=stage_rot)
                ax.set_yticks(np.arange(N_bio))
                ax.set_yticklabels([biomarker_labels[i] for i in biomarker_order], ha='right', fontsize=label_font_size, rotation=label_rot)
                for tick_label in ax.get_yticklabels():
                    tick_label.set_color(biomarker_colours[tick_label.get_text()])
                ax.set_xlabel(stage_label, fontsize=stage_font_size+2)
                ax.set_title(title_i, fontsize=title_font_size)
