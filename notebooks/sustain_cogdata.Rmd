...
```
```{r}
# interaction plot by facet
#png(file.path(dir_name, paste0(var, "_interaction_plot.png")), width = 800, height = 600)
#probe_interaction(model, pred = "stage", modx = "subtype", plot.points = TRUE, int.width = 0.95, interval = TRUE)
# save faucet plot
interact_plot(model, pred = "stage", modx = "subtype", plot.points = TRUE, int.width = 0.95, interval = TRUE,facet.modx=TRUE,linearity.check = TRUE)
```

## simple slope
```{R}
# Generate simple slopes analysis and save it
sim_slopes(model, pred = "stage", modx = "subtype")
```
...
ggplot() +
  ...
  labs(
    x = "Stage", y = var, color = "Subtype", fill = "Subtype"
  ) +
  scale_color_manual(values = c("dark blue", "dark green", "dark red")) +
  scale_fill_manual(values = c("dark blue", "dark green", "dark red")) +
  annotate("text", x = -Inf, y = -Inf, label = "S3 slope", 
           hjust = -0.1, vjust = -0.8, color = "dark red", size = 5, fontface = "bold") +
  annotate("text", x = -Inf, y = -Inf, label = "S2 slope", 
           hjust = -0.1, vjust = -2, color = "dark green", size = 5, fontface = "bold") + 
  annotate("text", x = -Inf, y = -Inf, label = "S1 slope", 
           hjust = -0.1, vjust = -3.5, color = "dark blue", size = 5, fontface = "bold") +
