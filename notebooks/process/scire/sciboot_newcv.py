...
if __name__ == "__main__": 
    ...
    rsl = np.random.randint(0, 9999999, size=n_re) 
    #this would ensure that they are using the same lists of random state 
    #each rs correspond to how the data index is sample
    #which would be applied to all roi the same way.
    #so as to subject level bootstrap
    for roi_idx, roi_data in enumerate(dat.T):
        ...
        gmmm = GaussianMixture(n_components=compk[roi_name],
                                 ...
                                 n_init=100,
                                 random_state=42)
        roi_boot_df = bootstrap_gmm_stats(gmmm, roi_data, n_re, rsl)
