...
setwd("/Users/linlin/Desktop/RabLab/eoad_sustain_ml/data")
```

# read
```{r readin} 
bldat <- read_csv(here("data","sustain_dat_baseline_filtered.csv"))
...
var_range <- read.csv(here("data","cognitive measures.csv"))
```
...
# longitudinal 
```{R}  
long <- read.csv(here("data",'combined_exclude_long_forR_longi.csv'))
var_list <- c('UDSBENTC','TRAILA','DIGFORCT','MINTTOTS','MOCATOTS','CDRSB','TRAILB','DIGBACCT')
# Loop through each variable
subject_counts <- list()
for (var_name in var_list) {
  var_file <- paste0(var_name, ".csv")  # Assign the file path
  print(var_name)  # Print progressß
  df <- read.csv(here("data",var_file))
  ...
  results_folder <- here("data",paste0("results08_", var_name))
  ...
  subject_counts[[var_name]] <- n_sub
   p <- ggplot() + 
    geom_line(data = df, aes(x = time_yr, y = .data[[var_name]], group = subj, color = subtype), size = 0.4, alpha = 0.3) +#"grey80"
  ...
  # Save plot
  ggsave(file.path(results_folder, paste0(var_name, "_longitudinal_plot_unannotated_colorized.png")), plot = p, width = 8, height = 6, dpi = 300,bg='#ffffff')
